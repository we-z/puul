    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            HStack(spacing: 0) {
                ...
                .frame(width: screenWidth)
                // 0.1 to 100
                // swiping from right to left goes from -screenwidth to 0
                .blur(
                    radius: {
                        let offset = currentOffset + dragOffset
                        // offset ranges from 0 to -screenWidth
                        // We'll convert that into a 0...1 ratio:
                        let ratio = -offset / screenWidth
